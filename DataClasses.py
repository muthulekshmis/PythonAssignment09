...
# Muthu , 2021-Mar-11 , Added code for Track class 
# Muthu , 2021-Mar-12 , Modifiled CD Class and added Track list to it 
# Muthu , 2021-Mar-12 , Added fiunctions in CD class for track addition/removal
# Muthu , 2021-Mar-13 , Handled exception for get_track function
#------------------------------------------#
...
class CD:
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        try:
#            if(int(track_id) == int(self.__tracks[int(track_id) - 1].position)):               
            if not(self.__tracks[int(track_id) - 1] == None):               
                del self.__tracks[int(track_id) - 1]
                self.__sort_tracks()
                print("Track {} deleted successfully " , format(track_id))
            else:
                print("No Such Track Found")
        except:
            print("Error deleting tracks : Possible cause No Such Track Found")
